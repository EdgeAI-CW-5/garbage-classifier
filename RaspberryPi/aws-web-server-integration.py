...
'''
Store Images: S3 bucker
Store Sensor Data: RDS - MySQL
...
# === Define open/close angles per bin ===
servo_angles = {
    "Plastic": {"open": 0, "closed": 90},
    "Metal": {"open": 0, "closed": 90},
    "Cardboard": {"open": 0, "closed": 90}
}

# Close all bins at the start
for label, pwm in servos.items():
    set_servo_angle(pwm, servo_angles[label]["closed"])
    time.sleep(0.5)

# === Ultrasonic for Plastic only ===
ultrasonic_trig = 23
ultrasonic_echo = 24
...
try:
    while True:
        ...
        pwm = servos[label]
        set_servo_angle(pwm, servo_angles[label]["open"])
        time.sleep(10)
        set_servo_angle(pwm, servo_angles[label]["closed"])
        print("?? Bin lid closed.")

        # === Bin level detection (only for Plastic) ===
        distance = None
        bin_status = ""
        ...
        try:
            api_url = "http://54.90.95.65/upload-data" 
            with open(filepath, 'rb') as img_file:
                files = {'image': img_file}
                data = {
                    'label': label,
                    'confidence': confidence,
                    'distance': str(distance) if label == "Plastic" and distance is not None else None,
                ...
                if response.status_code == 200:
                    ...
                    print(f"?? Failed to send data. Status: {response.status_code} | {response.text}")
