# 3 Servo Motors & # Ultrasonic Sensors - Didn't work properly

'''
Connected Devices to Raspberry pi:  3x SG90 Servo Motors (draw high current when moving),
                                    3x HC-SR04 Ultrasonic Sensors,
                                    1x USB Webcam (draws decent current via USB)

The Raspberry Pi’s onboard 5V rail or USB power input can’t supply enough current, 
so it: Freezes, Disconnects VNC, Auto reboots, Crashes camera or GPIO usage
...
ultrasonic_pins = {
    "Plastic": {"TRIG": 23, "ECHO": 24},
    "Metal": {"TRIG": 5, "ECHO": 6},
    "Cardboard": {"TRIG": 17, "ECHO": 27}
}

for pins in ultrasonic_pins.values():
...
try:
    while True:
        ...
        trig = ultrasonic_pins[label]["TRIG"]
        echo = ultrasonic_pins[label]["ECHO"]
        distance = get_bin_distance(trig, echo)

        if distance == -1:
            print(f"⚠️ {label.upper()} bin distance measurement failed.")
        else:
            print(f"📏 {label.upper()} bin level: {distance} cm")
            if distance < 10:
                print(f"⚠️ {label.upper()} bin is FULL!")
            else:
                print(f"✅ {label.upper()} bin has space.")
